...
- **CODE_REVIEW = true** — the configured Codex reviewer reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip. Passed through to `/experiment-bridge`.
...
- New run: `python3 "$RUN_STATE" start . "$RUN_ID" --executor "<actual-executor-model>" --provisional-advances --phases "idea-discovery,experiment-bridge,auto-review-loop,summary,paper-writing"` (the `--provisional-advances` policy is what lets a same-family provisional verdict close a phase for resume — without it, mainline semantics apply and provisional phases stay open).
- Resume: `python3 "$RUN_STATE" resume . "$RUN_ID"`; restart the returned phase.
- Each phase: mark `running`, then `done --artifact <path>`.
- A fresh Codex reviewer PASS uses `mark-provisional --reviewer "<actual-reviewer-model>"
...
3. **Fresh-agent code review** — the configured Codex reviewer reviews the implementation in a new context; base result is same-family provisional
...
1. the configured Codex reviewer reviews the work (score, weaknesses, minimum fixes)
2. the executing agent implements fixes (code changes, new experiments, reframing)
