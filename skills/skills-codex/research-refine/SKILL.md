...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Codex/Codex): Review for fidelity, specificity, contribution quality, and frontier leverage
...
Send the full proposal to the configured reviewer for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.

```
spawn_agent:
  task_name: research_refine_review
  fork_turns: none
  message: |
...
Send the revised proposal back to Codex in the **same agent**:
